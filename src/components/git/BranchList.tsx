...
const BranchList: Component<BranchListProps> = (props) => {
  ...
  const handleCheckout = async (name: string) => {
    ...
    try {
      ...
      addToast(`切换分支失败: ${message}`, 'error', detail);
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      ...
      const { message, detail } = describeErrorDetail(e);
      addToast(`删除分支失败: ${message}`, 'error', detail);
      setError(describeError(e));
    }
  };
  ...
  return (
    <div class="flex flex-col h-full relative">
      ...
      {/* Checkout toast */}
      <div
        class="absolute bottom-4 left-4 right-4 z-20 flex items-center gap-2 px-4 py-3 rounded-lg bg-cyan-900/90 border border-cyan-500/30 shadow-lg text-sm transition-all duration-150"
        classList={{
