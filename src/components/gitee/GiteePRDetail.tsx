...
import type { GiteePullRequest, GiteePullRequestFile } from '../../lib/types';
...
const GiteePRDetail: Component<Props> = (props) => {
  ...
  const checkMergeConflict = async () => {
    ...
    try {
      const freshPr = await giteeGetPull(props.owner, props.repo, pr().number);
  ...
  const [files] = createResource(
    () => pr().number,
    async (num) => {
      return giteeGetPullFiles(props.owner, props.repo, num);
    }
  );
  ...
  const [comments] = createResource(
    () => pr().number,
    async (num) => {
      return giteeListPullComments(props.owner, props.repo, num);
    }
  );
  ...
  const handleMerge = async () => {
    setMerging(true);
    try {
      const result = await giteeMergePull(props.owner, props.repo, pr().number, {
