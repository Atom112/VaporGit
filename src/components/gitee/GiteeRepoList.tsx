...
import { giteeListRepos } from '../../lib/tauriCommands';
import { giteeStore, cacheGiteeRepos } from '../../stores/giteeStore';
import type { GiteeRepo } from '../../lib/types';
...
const GiteeRepoList: Component<Props> = (props) => {
  const [search, setSearch] = createSignal('');
  const [page, setPage] = createSignal(1);
  const [repos, setRepos] = createSignal<GiteeRepo[]>(giteeStore.reposCache ?? []);
  ...
  const fetchRepos = async (p: number) => {
    ...
    setError(null);
    try {
      const list = await giteeListRepos(p, 50);
      setRepos(list);
      if (p === 1) cacheGiteeRepos(list);
    } catch (e) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  createEffect(() => {
    const p = page();
    if (p === 1 && giteeStore.reposCache) return;
    fetchRepos(p);
  });
  ...
  return (
    <div class="space-y-3 flex flex-col min-h-0">
      <input
        ...
        placeholder={tt('gitee.searchRepos')}
        value={search()}
      ...
      <Show when={loading() && !error()}>
        <div class="flex items-center justify-center py-8 shrink-0">
          ...
          <span class="ml-2 text-sm text-gray-400">{tt('gitee.loadingRepos')}</span>
        </div>
      </Show>
      ...
      <Show when={!loading() && !error() && filtered()?.length === 0}>
        <div class="text-center py-8 text-sm text-gray-500 shrink-0">
          {search() ? tt('gitee.noMatchRepos') : tt('gitee.noRepos')}
        </div>
      </Show>
      ...
      <Show when={repos().length >= 50}>
        <div class="flex justify-center gap-2 pt-2 shrink-0">
          ...
          <span class="px-2 py-1 text-xs text-gray-400">{ttf('gitee.page', page())}</span>
          <button
            class="px-3 py-1 text-xs rounded-lg bg-white/10 hover:bg-white/20 disabled:opacity-30 transition-colors"
...
const RepoCard: Component<{ repo: GiteeRepo; onClone: (url: string) => void }> = (props) => {
  const r = () => props.repo;
  const timeAgo = () => {
    const d = new Date(r().updatedAt);
    const diff = Date.now() - d.getTime();
    const hours = Math.floor(diff / 3600000);
    if (hours < 1) return tt('gitee.justNow');
    if (hours < 24) return ttf('gitee.hoursAgo', hours);
    const days = Math.floor(hours / 24);
    if (days < 30) return ttf('gitee.daysAgo', days);
    const months = Math.floor(days / 30);
    return ttf('gitee.monthsAgo', months);
  };
      ...
      <div class="flex items-start justify-between gap-2">
        <div class="min-w-0 flex-1">
          ...
          <div class="flex items-center gap-3 mt-1.5 text-[11px] text-gray-400">
            ...
            <span>{tt('gitee.updatedAt')} {timeAgo()}</span>
          </div>
        </div>
        <button
