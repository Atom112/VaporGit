...
import { giteeCreatePull, giteeListBranches } from '../../lib/tauriCommands';
...
const GiteePRCreateDialog: Component<Props> = (props) => {
  ...
  const [branches] = createResource(
    ...
    ([owner, repo]) => giteeListBranches(owner, repo)
  );
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const pr = await giteeCreatePull(props.owner, props.repo, {
        title: title().trim(),
        head: headBranch(),
        base: base(),
        body: body().trim() || null,
...
export default GiteePRCreateDialog;
